...
右键点击我的电脑 -> 属性 -> 高级系统设置 -> 环境变量，这次就不是新建自己的变量了，是把 bin 目录追加都 "Path" 变量的值中，给 "Path" 的值追加 "`%JAVA_HOME%/bin`"，注意多个路径之间都是用分号隔开的，看看原来的后面有没有分号，没有要先加上再追加自己的路径，分号都是英文分号。因为我们已经添加了 "`JAVA_HOME`" 变量，它的值也就是 jdk 的目录，所以可以直接引用这个变量。
